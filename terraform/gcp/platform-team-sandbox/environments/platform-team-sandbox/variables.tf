...
variable "project_id" {
  description = "GCP project ID (Platform Team Sandbox)"
  type        = string
}
...
variable "enable_autopilot" {
  description = "Enable GKE Autopilot mode (not recommended for sysbox)"
  type        = bool
  default     = false
}
...
# -----------------------------------------------------------------------------
# Runtime Node Pool Configuration (Sysbox)
# -----------------------------------------------------------------------------
# The runtime node pool runs with sysbox for container-in-container support.
...
variable "create_runtime_node_pool" {
  description = "Create dedicated node pool for runtimes with sysbox"
  type        = bool
  default     = true
}

variable "runtime_node_machine_type" {
  description = "Machine type for runtime nodes (needs sufficient resources for runtime containers)"
  type        = string
  default     = "e2-standard-8"
}

variable "runtime_node_disk_size_gb" {
  description = "Disk size in GB for runtime nodes (larger for runtime image caching)"
  type        = number
  default     = 200
}
...
variable "domain" {
  description = "Domain for the environment (e.g., ohe-staging.platform-team.all-hands.dev)"
  type        = string
}
