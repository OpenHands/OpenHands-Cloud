...
PROVIDERS = {
    "anthropic": ({"llm_provider": "anthropic", "anthropic_api_key": "k"}, "anthropic/"),
    "openai": ({"llm_provider": "openai", "openai_api_key": "k"}, "openai/"),
    "gemini": (
        {"llm_provider": "google", "google_api_type": "gemini", "google_gemini_api_key": "k"},
        "gemini/",
    ),
    "vertex": (
        {
    ...
    "openrouter": ({"llm_provider": "openrouter", "openrouter_api_key": "k"}, "openrouter/"),
    "bedrock-static-keys": (
        {
...
def _to_helm(template: str) -> str:
    ...
    return re.sub(r'ConfigOption(?:Data)? "([^"]+)"', r'(index $.Values.cfg "\1")', template)
...
def _config_options(overrides: dict[str, str]) -> dict[str, str]:
    ...
    return {name: ("" if str(visible.get(name)) == "false" else v) for name, v in cfg.items()}


@cache
def _optional_litellm_blocks() -> tuple[tuple[str, str | None, str | None], ...]:
    doc = next(
        d for d in yaml.safe_load_all(OPENHANDS.read_text(encoding="utf-8"))
        if isinstance(d, dict) and d.get("metadata", {}).get("name") == "openhands"
    )
    spec = doc["spec"]
    blocks = [("true", spec["values"]["env"]["LITELLM_DEFAULT_MODEL"],
    ...
    for entry in spec.get("optionalValues", []):
        ...
        model_list = values.get("litellm-helm", {}).get("proxy_config", {}).get("model_list")
...
def exported_secret_env(provider: str) -> frozenset[str]:
    ...
    cfg = _config_options(PROVIDERS[provider][0])
    doc = next(
        d for d in yaml.safe_load_all(SECRETS.read_text(encoding="utf-8"))
        if isinstance(d, dict) and d.get("kind") == "HelmChart"
    )
    template = SECRETS_CHART / "templates" / "litellm-env-secrets.yaml"
    used = set(re.findall(r"\.Values\.config\.(\w+)", template.read_text(encoding="utf-8")))
    config = _helm_render(
        {k: v for k, v in doc["spec"]["values"]["config"].items() if k in used}, cfg
    )
    result = subprocess.run(
        ["helm", "template", "openhands-secrets", str(SECRETS_CHART),
         "--show-only", "templates/litellm-env-secrets.yaml", "--values", "-"],
        check=True,
        capture_output=True,
        text=True,
        input=json.dumps({"config": {k: "" if v is None else str(v) for k, v in config.items()}}),
    )
...
def test_routes_use_only_the_selected_provider(provider: str) -> None:
    ...
    wrong = [r["litellm_params"]["model"] for r in routes
...
def test_route_credentials_are_exported_by_the_secret(provider: str) -> None:
    ...
    assert not missing, f"{provider}: routes read env vars the secret does not set: {missing}"
...
def test_default_model_is_a_visible_route(provider: str) -> None:
    ...
    visible = {
        r["model_name"] for r in routes or []
